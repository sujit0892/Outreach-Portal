...
public interface GroupMessageDao extends JpaRepository<GroupMessage,Integer> {
	...
	@Query("Select distinct g.group from GroupMessage g where g.user1=:user1 or g.user2=:user1 order by g.timestamp desc")
	List<Group> getRecentGroup(@Param("user1") User user);
	
	@Query("Select g from GroupMessage g where (g.user1=:user or g.user2=:user) and g.group=:group order by g.timestamp desc")
